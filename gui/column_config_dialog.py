...
class ColumnConfigPanel(QWidget):
    ...
    def __init__(self, table_config_manager, main_window=None, parent=None):
        ...
        Args:
            ...
            main_window: MainWindow reference (for analysis_results_df)
    ...
    def _init_ui(self):
        ...
        self.enable_all_additional_button.clicked.connect(
            self._on_enable_all_additional
        )
        self.disable_all_additional_button = QPushButton("Disable All")
        self.disable_all_additional_button.clicked.connect(
            self._on_disable_all_additional
        )
        ...
        self.reset_button.setToolTip(
            "Reset all columns to default visibility and order"
        )
    ...
    def _load_columns(self, config):
        ...
        if (
            hasattr(self.parent_window, "analysis_results_df")
            and self.parent_window.analysis_results_df is not None
        ):
            ...
            all_columns = (
                ...
                else list(config.visible_columns.keys())
            )
        ...
        for category in CATEGORY_ORDER:
            ...
            for col_name in cols_in_category:
                ...
                if col_name in config.locked_columns:
                    item.setToolTip(
                        f"{col_name} (locked column, always visible and first)"
                    )
                    font = item.font()
                    font.setBold(True)
                    item.setFont(font)
    ...
    def _on_save_view(self):
        ...
        view_name, ok = QInputDialog.getText(
            self, "Save View As", "Enter view name:", text=""
        )
        ...
        try:
            ...
            QMessageBox.information(
                self, "View Saved", f"View '{view_name}' has been saved successfully."
            )
        except Exception as e:
            logger.exception(f"Failed to save view '{view_name}'")
            QMessageBox.critical(self, "Save Failed", f"Failed to save view: {e!s}")
    ...
    def _on_delete_view(self):
        ...
        if not view_name or view_name == "Default":
            QMessageBox.warning(
                self, "Cannot Delete", "Cannot delete the Default view."
            )
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Delete Failed", f"Failed to delete view: {e!s}")
    ...
    def revert_config(self):
        ...
        if self._original_view_name and hasattr(
            self.parent_window, "current_client_id"
        ):
            client_id = self.parent_window.current_client_id
            if client_id:
                self.table_config_manager.load_config(
                    client_id, self._original_view_name
                )
    ...
    def apply_config(self):
        ...
        try:
            ...
            if (
                hasattr(self.parent_window, "current_client_id")
                and self.parent_window.current_client_id
            ):
                client_id = self.parent_window.current_client_id
                ...
                if (
                    hasattr(self, "additional_columns_config")
                    and self.additional_columns_config
                ):
                    logger.debug(
                        "Syncing UI checkbox states to config before saving..."
                    )
                ...
                if additional_columns is not None:
                    enabled_cols = [
                        col for col in additional_columns if col.get("enabled", False)
                    ]
                    disabled_cols = [
                        col
                    ...
                    logger.debug(
                        f"Saved additional columns config: {len(additional_columns)} columns"
                    )
                    logger.debug(
                        f"  Enabled: {len(enabled_cols)} - {[col['csv_name'] for col in enabled_cols]}"
                    )
                ...
                if additional_columns and any(
                    col.get("enabled", False) for col in additional_columns
                ):
                    QMessageBox.information(
                        self,
                        "Configuration Saved",
            ...
            QMessageBox.critical(
                self, "Apply Failed", f"Failed to apply configuration: {e!s}"
            )
    ...
    def _on_scan_csv(self):
        ...
        if (
            not hasattr(main_window, "last_loaded_orders_df")
            or main_window.last_loaded_orders_df is None
        ):
            QMessageBox.information(
                self,
                "No CSV Loaded",
        ...
        discovered = discover_additional_columns(
            orders_df, column_mappings, current_additional
        )
    ...
    def _populate_additional_columns_list(self, columns_config: list[dict]):
        ...
        for col_config in columns_config:
            ...
            order_level_cb.setToolTip(
                "If checked, this column will be forward-filled for multi-item orders"
            )
    ...
    def _load_additional_columns_config(self):
        ...
        if (
            not hasattr(self.parent_window, "current_client_id")
            or not self.parent_window.current_client_id
        ):
        ...
        try:
            ...
            if existing_additional:
                ...
                logger.info(
                    f"Loaded {len(existing_additional)} additional columns from client config"
                )
            else:
                logger.debug(
                    "No additional columns configuration found for this client"
                )
    ...
    def _on_additional_column_toggled(self, state):
        ...
        col = next(
            (
                c
                for c in self.additional_columns_config
        ...
        if col:
            ...
            logger.debug(
                f"Toggled '{csv_name}' ({internal_name}): enabled={is_enabled}"
            )
    ...
    def _sync_ui_to_config(self):
        ...
        if (
            not hasattr(self, "additional_columns_config")
            or not self.additional_columns_config
        ):
            return
        ...
        for col in self.additional_columns_config:
            ...
            if internal_name in ui_states:
                ...
                if old_enabled != new_enabled:
                    changes_count += 1
                    logger.debug(
                        f"  Synced '{col['csv_name']}': {old_enabled} -> {new_enabled}"
                    )
...
class ColumnConfigDialog(QDialog):
    ...
    def __init__(self, table_config_manager, parent=None, main_window=None):
        ...
        self.panel = ColumnConfigPanel(
            table_config_manager, main_window=main_window or parent, parent=self
        )
