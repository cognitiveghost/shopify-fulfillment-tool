...
class TestSimulateStockAllocationFifo:
    def test_backward_compat_no_fifo(self):
        ...
        results, lot_allocs, _ = _simulate_stock_allocation(
            orders, stock, prioritized, fifo_lots=None
        )
    ...
    def test_single_lot_fulfilled(self):
        ...
        results, lot_allocs, _ = _simulate_stock_allocation(
            orders, stock, prioritized, fifo_lots=fifo_lots
        )
        assert results["O1"]["fulfillable"] is True
            ...
            [
                {
                    "SKU": "A",
                    "Quantity": 5,
                    "Lot_Details": [{"expiry": "1", "batch": "B1", "qty_allocated": 5}],
                },
            ]
        )
        result = _expand_lot_summary(df)
...
class TestCreateStockExportLots:
    ...
    def test_lot_path_empty_df(self, tmp_path):
        ...
        df = pd.DataFrame(
            columns=["Order_Fulfillment_Status", "SKU", "Quantity", "Lot_Details"]
        )
            ...
            [
                {
                    ...
                    "SKU": "A",
                    "Quantity": 5,
                    "Lot_Details": [{"expiry": "1", "batch": "1", "qty_allocated": 5}],
                },
            ]
        )
        result = _expand_lot_rows(df)
